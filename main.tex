...
%  The class adds the "Abstract" heading automatically. Keep the body to a
%  single paragraph.
% -----------------------------------------------------------------------------
\begin{abstract}
This document is both a working example and a usage guide for the Yale arXiv paper template. It demonstrates how the template renders the title, authors, abstract, headings, citations, figures, tables, and mathematics, and it explains the document-class options and custom commands the template provides. To prepare your own paper, replace the title, author, abstract, and body text below with your own content while leaving the surrounding template machinery (\texttt{yalearxiv.cls}, \texttt{preamble.tex}, and \texttt{command.tex}) intact. The abstract should be limited to a single paragraph; it is typeset in $10$-point type inside the rounded title box at the top of the first page, and is optionally followed by a list of keywords.
\end{abstract}
